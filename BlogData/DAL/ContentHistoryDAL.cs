...
namespace BlogData.DAL
{
    public class ContentHistoryDal : GenericRepository<ContentHistory>
    {
        ...
        public IEnumerable<ContentHistory> GetPublishedPosts(IEnumerable<Content> blogContents)
        {
            ...
            return DbSet.ToList().Where(b => blogContents.Select(c => c.Id).Contains(b.ContentId))
                .GroupBy(b => b.ContentId)
                .Select(b => b.OrderByDescending(bp => bp.CreatedDate).First())
                .Where(b => b.ContentStateId.Equals(4));
        }
